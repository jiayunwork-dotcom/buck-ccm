...
// validBody is the preset example serialized as a request body.
const validBody = `{"vin":12,"d":0.4167,"l":0.0001,"c":0.00022,"ts":0.00001,"r":10}`

// TestModeHandlerOK posts a valid spec and expects a mode/Vout JSON reply.
...
// TestModeHandlerErrorJSON posts an invalid duty and expects error JSON from
// the backend with a 400 status.
...
// TestModeHandlerMethodNotAllowed verifies GET is rejected on POST endpoints.
...
// TestRippleHandlerPoints posts a valid spec and expects ripple numbers plus
// a backend-generated current waveform point list.
